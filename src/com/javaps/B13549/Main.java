...
public class Main {
    ...
    private static int BFS(int from, int to) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.idx == to) min = Math.min(min, node.time);
            if (node.idx * 2 <= MAX_RANGE && !visited[node.idx * 2]) queue.offer(new Node(node.idx * 2, node.time));
